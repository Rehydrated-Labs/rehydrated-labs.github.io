---
layout: wiki-article
---
# Welcome to the Quicksand Rehydrated Wiki!

Welcome to the wiki for [Quicksand Rehydrated]({% link pages/quicksand-rehydrated.md %}), where anyone can contribute via the [GitHub repository](https://github.com/Rehydrated-Labs/rehydrated-labs.github.io). The mod is still early in development, and in need of contributors. The developers host a [Discord server](https://discord.com/invite/RmpR94fXTb) for anyone who wants to contribute or keep track of the mod's progress.

## Companion mods

The mod currently has four planned companion mods beyond Quicksand Rehydrated:

- [Quicksand Core]({% link pages/quicksand-core.md %}) — A more modpack-friendly version of QSR with terrain-themed hazards & structrues and [Hunnibees]({% link pages/hunnibees.md %}).
- [Quicksand Glacéed]({% link pages/quicksand-glaceed.md %}) — An addon that includes a candy world dimension and expands QSR's optional content.
- Quicksand Laboratory — Adds an industrial structure similar to Minecraft's stronghold, which has new resources, machines, and hazards.
- Quicksand Carnivores — Adds a wider variety of mobs to the game.

## Mechanics

- Struggling
- Locating the Laboratory

## Items

- Long stick
- Rope
- Grappling Hook (and its components)
- Ball and Chain
- Ball and Chain Key
- Phoenix Feather
- Phoenix Feather charm
- Satchel of Returning
- Potion of Sinking
- Laboratory Map

## Blocks

- Mess Mixer
- Sludge Tank
- Spring
- Growth Lamp
- Reeds
- Cattails
- Cranberry bush
- Duckweed
- Muddy Sticks

## Mobs

- Cave Blob
- Mud Blob
- Tar Blob
- [Hunnibee]({% link pages/hunnibees.md %})
- Mudologer (Villager)
- Glowworm
- Webslinger (spider)
- Deep Worm
- Spittle Plant
- Rubberpot

## Substances

- [Mire]({% link pages/mire.md %})
- [Deep Mud]({% link pages/mud.md %})
- [Bog]({% link pages/bog.md %})
- [Peat Bog]({% link pages/peat_bog.md %})
- [Marsh]({% link pages/marsh.md %})
- Brown Clay
- Liquid Clay
- Grey Clay
- Mineral Clay
- Silt
- Dripstone Silt
- Quicksand
- Dry Quicksand
- Red Quicksand
- Jungle Quicksand
- Soft Quicksand
- Tar
- Wither Mud
- Corrupted Soulsand
- Warped Bog
- Crimson Clay
- Living Flesh
- Liquid Void
- Ender Mud
- Ender Clay
- Living Slime
- Liquid Honey
- Pillow Moss
- Tangleroot Moss
- Sculk Sludge
- Acid
- Mucus
- Dense Webbing
- Larva
- Tree Wax
- Wet Concrete
- Oobleck
- Glue Trap
- Liquid Rubber
- Sinking Wool
- Sinking Grain
- Muddy Rotplank
- Powdered Snow
- Synthetic Clay
- Liquid Chocolate
- Powdered Sugar
- Bread Batter
- Taffy Tar
- Molten Cheese

## Structures

- Quarry
- Quicksand Pyramid
- Abandoned Pier
- Mud Spa
- Concrete Mixer (village structure)
- "Undertale" (powder snow over hole)
- Laboratory
- Sinkhole
- Rubbery Squeeze
- Giant Cake

## Biomes

- Saccharine Sea
- Sugar Desert
- Marzipan Mesa
- Marshmallow Mountains
- Honey Swamp
- Cake Plains